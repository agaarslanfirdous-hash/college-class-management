...
?>
<h1 class="h3 mb-4">Rejections &amp; updates</h1>
<p class="text-muted small">This page mirrors what administration sees, limited to you: (1) <strong>class offers</strong> a teacher declined for a class you take, and (2) <strong>program-timetable</strong> slot unavailability, when a board cell matches your class code.</p>
<h2 class="h5 mt-3">1. Class offer — teacher explanation</h2>
<?php if ($rows === []): ?>
    <p class="text-muted">No class-offer rejections for your enrollments.</p>
<?php else: ?>
...
<h2 class="h5 mt-4">2. Program timetable — slot unavailability (same class code)</h2>
<?php if ($timetable_declines === []): ?>
    <p class="text-muted">No matching timetable slot declines, or the board is not in use for your classes.</p>
<?php else: ?>
